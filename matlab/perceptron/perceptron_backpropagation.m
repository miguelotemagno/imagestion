...
for n = 1:e
    if(Hi(n,1) > 0 || Si(n,1) > 0 || Vi(n,1) > 0)
        ...
        Ps(3,j) = Vi(n,1);
        Ts(1,j) = 1;
%         if(Hi(n,1) > 0.9 || Hi(n,1) < 0.2 || So(n,1) < 0.3)
%        if(So(n,1)>0.5)% || Hi(n,1)<0.7)
%             Ts(1,j) = 1;
%         else
%             Ts(1,j) = 0;
%         end
...
net1=newff(Pi,[3,3,1],{'tansig','tansig','tansig'},'trainbfg');
...
[net1,tr]=train(net1,Po,To);
 
% Creation network and training
net2=newff(Pe,[3,3,1],{'tansig','tansig','tansig'},'trainbfg');
